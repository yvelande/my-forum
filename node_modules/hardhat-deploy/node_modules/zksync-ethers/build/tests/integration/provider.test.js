...
describe('Provider', () => {
    ...
    describe('#constructor()', () => {
        it('Provider() should return a `Provider` connected to local network when URL is not defined', async () => {
            const provider = new src_1.Provider();
            const network = await provider.getNetwork();
            (0, chai_1.expect)(network.chainId).to.be.equal(9);
        });
    });
    describe('#getDefaultProvider()', () => {
        it('should return a provider connected to local network by default', async () => {
            const provider = src_1.Provider.getDefaultProvider();
            const network = await provider.getNetwork();
            (0, chai_1.expect)(network.chainId).to.be.equal(270);
        });
        ...
        it('should return a provider connected to Goerli network', async () => {
            const provider = src_1.Provider.getDefaultProvider(src_1.types.Network.Goerli);
            const network = await provider.getNetwork();
            (0, chai_1.expect)(network.chainId).to.be.equal(280);
        });
        it('should return a provider connected to Sepolia network', async () => {
            const provider = src_1.Provider.getDefaultProvider(src_1.types.Network.Sepolia);
            const network = await provider.getNetwork();
            (0, chai_1.expect)(network.chainId).to.be.equal(300);
        });
        it('should return a provider connected to main network', async () => {
            const provider = src_1.Provider.getDefaultProvider(src_1.types.Network.Mainnet);
            const network = await provider.getNetwork();
            (0, chai_1.expect)(network.chainId).to.be.equal(324);
        });
    });
    ...
    describe('#getTestnetPaymasterAddress()', () => {
        it('should return the address of testnet paymaster', async () => {
            ...
            (0, chai_1.expect)(result).to.be.equal(TESTNET_PAYMASTER);
        });
    });
    describe('#l1ChainId()', () => {
        it('should return L1 chain ID', async () => {
            ...
            (0, chai_1.expect)(result).to.be.equal(L1_CHAIN_ID);
        });
    });
    ...
    describe('#getBalance()', () => {
        it('should return balance of the account at `address`', async () => {
            ...
            (0, chai_1.expect)(result.gte(ethers_1.BigNumber.from(0))).to.be.true;
        });
        it('should return a DAI balance of the account at `address`', async () => {
            const result = await provider.getBalance(ADDRESS, 'latest', await provider.l2TokenAddress(DAI_L1));
            (0, chai_1.expect)(result.isZero()).to.be.false;
        });
    });
    describe('#getAllAccountBalances()', () => {
        it('should return all balances of the account at `address`', async () => {
            ...
            (0, chai_1.expect)(Object.keys(result)).to.have.lengthOf(2); // ETH and DAI
        });
    });
    ...
    describe('#getBytecodeByHash(txHash)', () => {
        it('should return bytecode of a contract', async () => {
            ...
            const tokenFactoryDepsHash = ethers_1.ethers.utils.hexlify(src_1.utils.hashBytecode(testnetPaymasterBytecode)); // "0x0100012539970918b0ab79e00660723a76fed0d6dac333c9298fdbb8628407f2";
            const result = await provider.getBytecodeByHash(tokenFactoryDepsHash);
            (0, chai_1.expect)(result).to.be.deep.equal(Array.from(ethers_1.ethers.utils.arrayify(testnetPaymasterBytecode)));
        });
    });
    ...
    describe('#getProof()', () => {
        it('should return a storage proof', async () => {
            ...
            const addressPadded = ethers_1.ethers.utils.hexZeroPad(wallet.address, 32);
            // Convert the slot number to a hex string and pad it to 32 bytes
            const slotPadded = ethers_1.ethers.utils.hexZeroPad(ethers_1.ethers.utils.hexlify(0), 32);
            ...
            const storageKey = ethers_1.ethers.utils.keccak256(concatenated);
            const l1BatchNumber = await provider.getL1BatchNumber();
            try {
                const result = await provider.getProof(src_1.utils.NONCE_HOLDER_ADDRESS, [storageKey], l1BatchNumber);
                (0, chai_1.expect)(result).not.to.be.null;
            }
            catch (error) {
                // skip
    ...
    describe('#getTransactionStatus()', () => {
        ...
        it('should return the `NotFound` status for a non-existing transaction', async () => {
            ...
            (0, chai_1.expect)(result).to.be.equal(src_1.types.TransactionStatus.NotFound);
        });
    });
    ...
    describe('#newFilter()', () => {
        it('should return new filter', async () => {
            const result = await provider.newFilter({
                fromBlock: 0,
                toBlock: 5,
                address: src_1.utils.L2_ETH_TOKEN_ADDRESS,
            });
            (0, chai_1.expect)(result).not.to.be.null;
        });
    });
    ...
    describe('#l2TokenAddress()', () => {
        it('should return L2 token address', async () => {
            const result = await provider.l2TokenAddress(src_1.utils.ETH_ADDRESS);
            (0, chai_1.expect)(result).to.be.equal(src_1.utils.ETH_ADDRESS);
        });
    ...
    describe('#getWithdrawTx()', () => {
        it('should return withdraw transaction', async () => {
            const tx = {
                ...
                to: ethers_1.ethers.utils.getAddress('0x000000000000000000000000000000000000800a'),
            ...
            (0, chai_1.expect)(result).to.be.deep.equal(tx);
        });
        ...
        it('should return a DAI withdraw transaction', async () => {
            const tx = {
                from: ADDRESS,
                to: ethers_1.ethers.utils.getAddress((await provider.getDefaultBridgeAddresses()).erc20L2),
            ...
            (0, chai_1.expect)(result).to.be.deepEqualExcluding(tx, ['data']);
        });
        ...
        it('should throw an error when `tx.to` and `tx.from` are not provided', async () => {
            try {
                ...
                (0, chai_1.expect)(e).not.to.be.equal('withdrawal target address is undefined');
            }
        });
        it('should throw an error when `tx.amount!=tx.overrides.value', async () => {
            try {
                ...
                (0, chai_1.expect)(e).not.to.be.equal('The tx.value is not equal to the value withdrawn');
            }
        });
    });
    ...
    describe('#estimateFee()', () => {
        it('should return gas estimation of transaction', async () => {
            const result = await provider.estimateFee({
                from: ADDRESS,
                to: RECEIVER,
                value: ethers_1.BigNumber.from(7000000000).toHexString(),
            });
            (0, chai_1.expect)(result).not.to.be.null;
        });
    });
    describe('#estimateGas()', () => {
        it('should return gas estimation of transaction', async () => {
            const result = await provider.estimateGas({
                from: ADDRESS,
                to: await provider.l2TokenAddress(DAI_L1),
                data: src_1.utils.IERC20.encodeFunctionData('approve', [RECEIVER, 1]),
            });
            (0, chai_1.expect)(result.gt(ethers_1.BigNumber.from(0))).to.be.true;
        });
