...
const ethProvider = ethers_1.ethers.getDefaultProvider('http://127.0.0.1:8545');
...
async function deployPaymasterAndToken() {
    ...
    await faucetTx.wait();
    if (ethers_1.ethers.utils.getAddress(TOKEN) !== ethers_1.ethers.utils.getAddress(tokenAddress)) {
        throw new Error('token addresses mismatch');
    }
    if (ethers_1.ethers.utils.getAddress(PAYMASTER) !==
        ethers_1.ethers.utils.getAddress(paymasterAddress)) {
        throw new Error('paymaster addresses mismatch');
    }
    return { token: tokenAddress, paymaster: paymasterAddress };
}
async function createTokenL2(l1TokenAddress) {
